...
**Manual fallback only:** [Open RHMRA Phone Dashboard directly](https://abiemann.github.io/RobinhoodEquityTradingDashboardViewer/)
...
The **View on Phone** QR code carries the public viewer address and private pairing fragment, so normal setup does not require the user to find or type a web address. The direct public URL and **Copy private link** are backup options when QR scanning is unavailable.
...
On iPhone and iPad, Safari and the installed Home Screen app keep separate private storage. Install RHMRA before pairing. If scanning the QR code opens Safari instead of the installed app, use the backup approach below to paste the private pairing link inside the installed RHMRA app.

### Backup: install first and paste the private link
...
The dashboard QR flow is the recommended approach; the public URL and copied private link are the backup. Never post or forward the QR code or private link: it contains the dashboard decryption key.
...
- test the direct public URL plus **Copy private link** only as the documented fallback when QR scanning cannot complete;
...
The complete link encoded in the laptop's **View on Phone** QR code must append the exact fragment to the deployed PWA base URL. **Copy private link** exposes the same link only as the manual fallback. The AES key belongs only in the fragment; never put it in a query string, file, API request, or log.
